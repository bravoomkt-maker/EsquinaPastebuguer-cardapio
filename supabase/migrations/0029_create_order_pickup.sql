create or replace function public.create_order(
  ...
  p_notes text,
  p_items jsonb,
  p_order_type text default 'entrega'
)
...
as $$
declare
  v_order_type text := coalesce(nullif(trim(p_order_type), ''), 'entrega');
  ...
  v_item_name text;
  v_short_category record;
begin
  if v_order_type not in ('entrega', 'retirada') then
  ...
  insert into public.orders (
    ...
    subtotal, delivery_fee, total, order_type
  ) values (
    v_order_id,
    trim(p_customer_name),
    trim(p_customer_phone),
    case when v_order_type = 'entrega' then trim(p_street) else null end,
    case when v_order_type = 'entrega' then trim(p_number) else null end,
    nullif(trim(coalesce(p_complement, '')), ''),
    nullif(trim(coalesce(p_reference_point, '')), ''),
    case when v_order_type = 'entrega' then p_neighborhood_id else null end,
    p_payment_method,
    p_change_for,
  ...
  for v_short_category in
    select c.name as category_name, c.min_quantity as required, sum(oi.quantity) as total_qty
    from public.order_items oi
    join public.products p on p.id = oi.product_id
    join public.categories c on c.id = p.category_id
    where oi.order_id = v_order_id
    group by c.id, c.name, c.min_quantity
    having sum(oi.quantity) < c.min_quantity
  loop
    raise exception 'Categoria "%" precisa de pelo menos % unidades no total (você tem %)',
      v_short_category.category_name, v_short_category.required, v_short_category.total_qty;
  end loop;
