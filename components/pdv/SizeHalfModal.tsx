...
export function SizeHalfModal({
  ...
  const primaryPrice = primarySize ? (primarySize.promo_price ?? primarySize.price) : 0;

  const secondProduct = eligibleSecondProducts.find((p) => p.id === secondProductId);
  const secondSize = secondProductId
    ? (sizesByProduct[secondProductId] ?? []).find((s) => s.label === sizeLabel)
    : undefined;
  const secondPrice = secondSize ? (secondSize.promo_price ?? secondSize.price) : null;
